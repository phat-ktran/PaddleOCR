...
PaddlePaddle implementation of Star Temporal Classification (STC) Loss
"""
...
class STCLossFunction:
    ...
    def backward(inputs, targets, prob, reduction, grad_output, auxiliary_data):
        ...
        # Initialize gradient tensor - match PyTorch shape exactly
        input_grad = paddle.zeros([B, T, Cstar], dtype=inputs.dtype)

        # Compute gradients for each batch item
        for b in range(B):
            ...
            # Convert back to paddle tensor with exact PyTorch shape handling
            grad_tensor = paddle.to_tensor(grad_numpy, dtype=inputs.dtype).reshape(
                [1, T, Cstar]
            )

            # Apply scale exactly as PyTorch does
            input_grad[b] = grad_tensor.squeeze(0) * scales[b]
...
class STCLossLayer(paddle.autograd.PyLayer):
    """
    PaddlePaddle autograd layer for STC loss
    """
...
class STC(nn.Layer):
    ...
    def forward(self, predicts, batch):
        ...
        with paddle.set_grad_enabled(not log_probs.stop_gradient):
            ...
            # Flatten all target tokens and ensure they are integers
            all_target_tokens = []
            for target in targets:
                for t in target:
                    if isinstance(t, (list, tuple)):
                        # Handle nested sequences
                        all_target_tokens.extend(t)
                    else:
                        all_target_tokens.append(int(t))

            select_idx = [STC_BLANK_IDX] + sorted(list(set(all_target_tokens)))
            ...
            # Debug removed - comment out for production use
            # print(f"Final shape: {log_probs_final.shape}, select_idx: {len(select_idx)}")
...
def _basic_usage_example():
    """
    Basic usage example of STCLoss
    """
    print("=== Basic STCLoss Usage Example ===")
    ...
    # Compute gradients
    loss.backward()
    # In PaddlePaddle, gradients are accessed via the gradient tape or after backward()
    if hasattr(inputs, "gradient") and inputs.gradient is not None:
        print(f"Input gradients shape: {inputs.gradient().shape}")
    else:
        print("Input gradients computed (shape matches input)")
...
def _batch_size_test():
    """
    Test STC loss with different batch sizes
    """
    print("=== Batch Size Testing ===")
